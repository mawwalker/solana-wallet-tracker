    ...
    if update.effective_user.id in ALLOWED_USER_IDS:
        ...
        if len(args) == 2:
            ...
            if not await manager.is_subscribed(update.effective_user.id, wallet_address):
                subscribe_message = {"action": "subscribe", "user_id": update.effective_user.id, "wallet_address": wallet_address, "nickname": nickname}
                await manager.handle_external_message(subscribe_message)
...
async def remove_wallet(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if update.effective_user.id in ALLOWED_USER_IDS:
        ...
        if wallet_address:
            unsubscribe_message = {"action": "unsubscribe", "user_id": update.effective_user.id, "wallet_address": wallet_address}
            await manager.handle_external_message(unsubscribe_message)
...
async def main() -> None:
    ...
    result = await application.bot.set_my_commands(commands)
    ...
    async with application:
        ...
        connect_websocket_task = asyncio.create_task(manager.connect_websocket())

        # 等待 WebSocket 连接建立
        await manager.websocket_connected.wait()
        await manager.restore_subscriptions(application.bot)
